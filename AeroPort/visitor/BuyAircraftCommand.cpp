...
void BuyAircraftCommand::visit(AirportAuthority& a) {
	...
	std::println("[System] Purchased {}(ID: {}). {} balance : {:.2f} EUR.", airplane->getType(), airplane->getID() , airline->getName(), airline->getBalance());

	airline->addAirplane(std::move(airplane));
}